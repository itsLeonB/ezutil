...
func TestNewJwtService(t *testing.T) {
	...
	service := ezutil.NewJwtService(auth)
...
func TestJWTService_VerifyToken(t *testing.T) {
	...
	t.Run("token with wrong secret", func(t *testing.T) {
		...
		wrongAuth := &ezutil.Auth{
			SecretKey:      "wrong-secret-key",
			TokenDuration:  time.Hour,
			Issuer:         "test-issuer",
		}
		wrongService := ezutil.NewJwtService(wrongAuth)
	...
	t.Run("expired token", func(t *testing.T) {
		...
		shortAuth := &ezutil.Auth{
			SecretKey:      "test-secret-key-for-jwt-testing",
			TokenDuration:  time.Nanosecond, // Very short duration
			Issuer:         "test-issuer",
		}
		shortService := ezutil.NewJwtService(shortAuth)
		...
		appErr, ok := err.(ezutil.AppError)
		assert.True(t, ok)
		assert.Equal(t, "UnauthorizedError", appErr.Type)
	})
...
func TestNewHashService(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			service := ezutil.NewHashService(tt.cost)
			assert.NotNil(t, service)
...
func TestHashService_Hash(t *testing.T) {
	service := ezutil.NewHashService(4) // Low cost for faster tests
